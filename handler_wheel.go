...
func (self *TouchHandler) loop_handel_wasd_wheel() {
	...
	// [V3.4.6] 隔离的贝塞尔专用噪点相位计数器和淡入因子
	// 彻底解决由于共享全局 counter 导致的行星波噪点被贝塞尔参数污染、终点异常蠕动的问题
		...
		default:
			...
			// 模式 2: 内置高级贝塞尔模式
				...
				if target_changed || (is_moving && self.wasd_wheel_released) {
					...
					// 2. 确定终点 P3
					end_offset_x, end_offset_y := int32(0), int32(0)
					if self.wheel_random_point_enable {
						shift_active := self.shift_state
						walk_mode := self.wheel_walk_mode_enable
						is_shift := (shift_active && !walk_mode) || (!shift_active && walk_mode)
						
						if is_shift {
							end_offset_x, end_offset_y = self.get_random_offset(self.wheel_random_shift_end_radius_px)
						} else {
							end_offset_x, end_offset_y = self.get_random_offset(self.wheel_random_end_radius_px)
						}
					}
					bezier_p3_x = float64(wasd_target_x + end_offset_x)
					bezier_p3_y = float64(wasd_target_y + end_offset_y)
				...
				if is_moving {
					if bezier_t < 1.0 {
						...
						// 终点停泊：使用局部变量锁定，无视行星模式造成的全局时间扭曲
						self.wasd_wheel_last_x = int32(bezier_p3_x)
						self.wasd_wheel_last_y = int32(bezier_p3_y)
